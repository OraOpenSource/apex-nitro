...
module.exports = function () {
	...
	const launchPublish = async function (password) {
		launch(async () => {
			...
			// Call publish on template
			if (config.mode === 'pro') {
				const {productionBuild} = require(path.resolve(process.cwd(), 'nitro', 'nitro-build.js'));
				doPublish = await productionBuild();
			}

			// Run publish only, if template has indicated so
			if (doPublish) {
				publisher.publish({
