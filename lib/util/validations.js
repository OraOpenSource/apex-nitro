...
module.exports = {
	...
	srcDistFolders(config) {
		if (config.srcFolder === config.distFolder) {
			throw new Error(`Your project's source folder (${config.srcFolder}) cannot be equal to your distributable folder (${config.distFolder}).`);
		}
	},
	...
	isSrcFolderValid(config) {
		...
		try {
			...
			if (files.length === 0) {
				console.error(`Your source folder contains zero file: ${config.srcFolder}. Nothing to do...`);
			}

			return files.length > 0;
		} catch (error) {
			console.error(`Error while reading the source folder: ${config.srcFolder}. Please review the path.`);
			return false;
		}
	},
