...
module.exports = {
    ...
    connectStringRequired: function (project) {
        try {
            if (!userConfig[project].sqlcl.connectString == undefined)
                throw new Error('Whoops!');
        } catch (e) {
            console.error(chalk.red.bold("This project is missing the 'Connection String' parameter in order to publish the files."));
            console.error("Type", chalk.cyan.bold("afeb config " + project), "to edit it. It's under the Advanced section.");
            process.exit();
        }
    },

    // validates missing project header package.json path
    apexVersionRequired: function (project) {
        try {
            if (!userConfig[project].sqlcl.apexVersion == undefined)
                throw new Error('Whoops!');
        } catch (e) {
            console.error(chalk.red.bold("This project is missing the 'APEX Version' parameter in order to publish the files."));
            console.error("Type", chalk.cyan.bold("afeb config " + project), "to edit it. It's under the Advanced section.");
            process.exit();
        }
    }
