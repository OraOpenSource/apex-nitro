...
module.exports = {
	header(config) {
		if (config.header && config.header.enabled) {
			try {
				...
				throw new Error('Your package.json path is invalid.');
			}
		}
	},
	...
	publish(config) {
		const errorMessage = function (key) {
			return `Your apexnitro.config.json is missing ${key} information. Please review your ${path.resolve(process.cwd(), 'apexnitro.config.json')}`;
		};
