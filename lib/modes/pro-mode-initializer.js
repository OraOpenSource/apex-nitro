...
module.exports = class ProModeInitializer {
	...
	async init(config, configFileName) {
		...
		let spinner = ora({ text: 'Preparing project', spinner: 'dots' }).start();
		await templateService.installTemplate(this.repositoryUrl || this.templateName, this.getTempPath());
		...
		spinner = ora({ text: `Saving configuration to "${configFileName}"`, spinner: 'dots' }).start();
		const finalConfig = { ...config, ...proConfig, ...browserSyncConfig, ...publishConfig };
		await fs.writeFile(
		...
		spinner = ora({ text: 'Installing dependencies', spinner: 'dots' }).start();
