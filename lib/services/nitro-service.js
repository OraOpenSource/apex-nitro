...
const runCommand = async (webpack, config) => {
	return new Promise((resolve, reject) => {
		webpack(config, (err, stats) => {
			...
			if (stats.hasErrors() || stats.hasWarnings()) {
				...
				if (stats.toString().includes('Module build failed') ||
					stats.toString().includes('Cannot find module') ||
					stats.toString().includes('Module not found')) {
...
const build = async (config, firstAttempt = true, nomin = false) => {
	...
	try {
		...
		try {
			let configDev = require(path.resolve(process.cwd(), 'webpack.dev'));
			await runCommand(webpack, configDev);
		} catch (error) {
			catchError(error);
		}
		...
		if (buildResult.valid && nomin === false) {
			try {
				let configProd = require(path.resolve(process.cwd(), 'webpack.prod'));
				await runCommand(webpack, configProd);
			} catch (error) {
				catchError(error);
			}
		}
