...
module.exports = function (config, browsersync) {
	...
	gulp.task('style', () => {
		...
		const sourceStream = gulp.src(sourceFiles)
			...
			.pipe(plugins.if(config.css.concat, plugins.concat(util.padStr(config.css.concatFilename, '.css'))))
			.pipe(plugins.if(config.header.enabled, plugins.header(banner, {
				pkg
			})))
			.pipe(plugins.autoprefixer(options.autoprefixerOptions()));
		...
		return merge(unmin, min)
			...
			.pipe(plugins.if(config.browsersync.realTime, browsersync.stream({
				match: '**/*.css'
			})));
	});

	// Javascript & browsersync
	gulp.task('style-watch', gulp.series('style', done => {
		if (taskSuccess) {
