...
module.exports = function(gulp, config, browsersync) {
    ...
    gulp.task('style', function() {
        ...
        if (config.sass.enabled) {
            sourceFiles = [
                config.paths.src + config.assets.scss + config.files.scss,
                config.paths.src + config.assets.sass + config.files.sass
            ];
        } else if (config.less.enabled) {
            sourceFiles = config.paths.src + config.assets.less + config.files.less;
        } else {
            sourceFiles = config.paths.src + config.assets.css + config.files.css;
        }
        ...
        return merge(unmin, min)
            ...
            .pipe(plugins.if(config.browsersync.enabled, browsersync.stream({match: '**/' + config.files.css})));
    });
}
