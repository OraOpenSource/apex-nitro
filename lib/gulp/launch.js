...
module.exports = function (config, project, cb) {
	...
	gulp.task('launch', () => {
		...
		console.log(chalk.cyan(`is now launching`), chalk.cyan.bold(project), chalk.cyan('...'));
		...
		gulp.series(
			...
			gulp.parallel(tasks),
				...
				if (typeof cb === 'function') {
					...
					console.log(chalk.cyan.bold('All files in that directory sync to your APEX app.'));
