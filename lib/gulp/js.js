...
module.exports = function (config, browsersync) {
	...
	gulp.task('js', () => {
		...
		return gulp.src(path.resolve(config.srcFolder, 'js', '**/*.js'))
			...
			.pipe(plugins.if(config.js.jsConcat, plugins.concat(util.padStr(config.js.jsConcatFilename, '.js'))))
			.pipe(plugins.if(config.header.enabled, plugins.header(banner, {
				pkg
			})))
			.pipe(plugins.sourcemaps.write('./'))
			.pipe(plugins.if(isSuccess, gulp.dest(path.resolve(config.distFolder, 'js'))))
			.pipe(f)
